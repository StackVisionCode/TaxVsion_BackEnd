...
OpenTelemetry, y el microservicio de Subscription con facturacion por asientos
al estilo Google Workspace sin prorrateo.
...
+---------+---------+  +---------+---------+  +----------+-----------+
 ...
 TaxVision_Tenants        TaxVision_Auth          TaxVision_Subscription
  ...
  RabbitMQ exchange: taxvision-events
     ...
     |                                  |
     v                                  v
  queue: auth-tenant-events    queue: subscription-service-events
          |                                  |
          v                                  v
  Auth tenant registry            Subscription consumers
...
- enruta `/auth/*`, `/tenants/*`, `/enrollments/*`, `/subscriptions/*`, `/api/plans/*`, `/api/modules/*` y `/api/subscription-modules/*`;
- valida JWT en rutas protegidas; `enrollments` y lectura de planes y modulos son anonimas;
...
`taxvision-events` es un exchange fanout.
...
El consumidor de creacion de tenant en Subscription usa upsert. Ambos servicios usan
