...
public sealed class TenantEntitlementSnapshotTests
{
    ...
    public void Rebuild_increments_the_revision_number()
    {
        ...
        {
            new(EntitlementKey.Create("seats.max").Value, EntitlementValueType.Int, "5", EntitlementStatus.Active, EntitlementSource.Plan, null),
        };

        var result = TenantEntitlementSnapshot.Rebuild(
            tenantId, previousRevisionNumber: 3, "starter", Guid.NewGuid(), "Active", seatCount: 2, availableSeatCount: 1, entries, DateTime.UtcNow);
    ...
    {
        var snapshot = TenantEntitlementSnapshot.Rebuild(
            Guid.NewGuid(), 0, "starter", Guid.NewGuid(), "Active", 0, 0, [], DateTime.UtcNow).Value;
    ...
    {
        var result = TenantEntitlementSnapshot.Rebuild(
            Guid.NewGuid(), 0, "starter", Guid.NewGuid(), "Active", seatCount: -1, availableSeatCount: 0, [], DateTime.UtcNow);
