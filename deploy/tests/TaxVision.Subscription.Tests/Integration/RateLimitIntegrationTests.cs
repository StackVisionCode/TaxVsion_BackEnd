...
namespace TaxVision.Subscription.Tests.Integration;

/// <summary>
/// Fase 4.10 del plan de rate limiting (Plan_Implementacion_Fases.md §4) — prueba end-to-end real
/// contra Subscription.Api: SQL Server/Redis/RabbitMQ locales reales, sin mocks de
/// infraestructura. Un test por categoría real representativa del inventario de este servicio (F,
/// G) — mismo criterio de cobertura que
/// <c>TaxVision.Correspondence.Tests.Integration.RateLimitIntegrationTests</c> (Fase 4.9). El resto
/// de los 27 endpoints no-exentos queda cubierto por inspección de código + el mismo evaluador ya
/// verificado en Fase 4.1-4.9. <c>AddOnsController.Cancel</c> (G) se prueba sobre un id sintético
/// que no existe — el filtro de rate limit corre ANTES del handler, así que el 404/error de
/// negocio no bloquea el conteo, mismo criterio ya usado en fases previas. Cada test method usa un
...
/// </summary>
public sealed class RateLimitIntegrationTests : IClassFixture<SubscriptionApiFactory>
{
    private readonly SubscriptionApiFactory factory;

    public RateLimitIntegrationTests(SubscriptionApiFactory factory) => this.factory = factory;

    [Fact]
    public async Task GetEntitlementSummary_trips_with_user_layer_and_limit_300()
    {
        var tenantId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        var client = CreateAuthenticatedClient(tenantId, userId);

        var tripped = await FireUntilTrippedAsync(() => client.GetAsync("/entitlements/summary"), maxAttempts: 600);

        await AssertTripped(
            tripped,
            expectedPolicy: "subscription.f.entitlement_read",
            expectedLayer: "user",
            expectedLimit: 300
        );
    }
    ...
    [Fact]
    public async Task CancelAddOn_trips_with_user_layer_and_limit_60()
    {
        var tenantId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        ...
        var tripped = await FireUntilTrippedAsync(
            () => client.PostAsJsonAsync($"/addons/{addOnId}/cancel", new { Reason = "test" }),
            maxAttempts: 120
        );

        await AssertTripped(
            tripped,
            expectedPolicy: "subscription.g.addon_manage",
            expectedLayer: "user",
            expectedLimit: 60
        );
    }
