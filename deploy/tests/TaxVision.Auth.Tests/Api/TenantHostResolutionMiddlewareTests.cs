...
public sealed class TenantHostResolutionMiddlewareTests
{
    ...
    public async Task Known_host_resolves_and_calls_next()
    {
        ...
        var (middleware, resolver, tenantContext, audit, unitOfWork, nextCalled) = BuildMiddleware(enforce: true);
        ...
        await InvokeAsync(middleware, context, resolver, tenantContext, audit, unitOfWork);
    ...
    {
        var (middleware, resolver, tenantContext, audit, unitOfWork, nextCalled) = BuildMiddleware(enforce: false);
        resolver.ResolveFn = _ => HostResolutionResult.Unresolved(TenantResolutionFailureReason.HostUnknown);
