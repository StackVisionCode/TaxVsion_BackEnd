...
internal sealed class FakeStockRepository : IStockRepository
{
    ...
    public Task AddStockLevelAsync(StockLevel level, CancellationToken ct = default) { Levels.Add(level); return Task.CompletedTask; }
    ...
    public Task<(IReadOnlyList<StockLevel> Items, int Total)> ListStockLevelsAsync(Guid tenantId, bool lowStockOnly, int page, int pageSize, CancellationToken ct = default)
    {
        var q = Levels.Where(s => s.TenantId == tenantId);
        if (lowStockOnly) q = q.Where(s => s.IsLowStock);
        var all = q.ToList();
        IReadOnlyList<StockLevel> paged = all.Skip((page - 1) * pageSize).Take(pageSize).ToList();
        return Task.FromResult((paged, all.Count));
    }

    public Task<(IReadOnlyList<StockMovement> Items, int Total)> ListMovementsAsync(Guid tenantId, Guid? catalogItemId, int page, int pageSize, CancellationToken ct = default)
    {
        var q = Movements.Where(m => m.TenantId == tenantId);
        if (catalogItemId is { } cid) q = q.Where(m => m.CatalogItemId == cid);
        var all = q.ToList();
...
internal sealed class FakeSupplierRepository : ISupplierRepository
{
    ...
    public Task AddAsync(Supplier supplier, CancellationToken ct = default) { Store.Add(supplier); return Task.CompletedTask; }
}
...
internal sealed class FakeItemSupplierRepository : IItemSupplierRepository
{
    ...
    public Task<ItemSupplier?> GetAsync(Guid tenantId, Guid catalogItemId, Guid supplierId, CancellationToken ct = default) =>
        Task.FromResult(Store.FirstOrDefault(x => x.TenantId == tenantId && x.CatalogItemId == catalogItemId && x.SupplierId == supplierId));
    public Task<IReadOnlyList<ItemSupplier>> ListByItemAsync(Guid tenantId, Guid catalogItemId, CancellationToken ct = default)
    {
        IReadOnlyList<ItemSupplier> r = Store.Where(x => x.TenantId == tenantId && x.CatalogItemId == catalogItemId).ToList();
        return Task.FromResult(r);
    }
    public Task AddAsync(ItemSupplier link, CancellationToken ct = default) { Store.Add(link); return Task.CompletedTask; }
