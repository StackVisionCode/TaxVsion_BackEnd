...
/// <summary>
/// Mintea un JWT HS256 firmado con el secreto de test de <see cref="CustomerApiFactory"/> (nunca
...
/// <c>Authorization:PermissionsSource=Projection</c> (ver <c>ProjectionPermissionsSource.HasPermissionAsync</c>,
/// que chequea <c>IsPlatformAdmin()</c> antes que la proyección), así el test no depende de que
/// exista una fila de <c>UserPermissionsProjection</c> para el usuario sintético.
/// </summary>
public static class JwtTestTokenFactory
{
    public static string Mint(CustomerApiFactory factory, Guid tenantId, Guid userId)
    {
