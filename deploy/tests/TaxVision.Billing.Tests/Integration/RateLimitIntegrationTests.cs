...
/// <summary>
/// Fase 4.11 del plan de rate limiting (Plan_Implementacion_Fases.md §4) — prueba end-to-end real
/// contra Billing.Api: SQL Server/Redis/RabbitMQ locales reales, sin mocks de infraestructura. Un
/// test por categoría real de este servicio (F, G) — mismo criterio de cobertura que
/// <c>TaxVision.Subscription.Tests.Integration.RateLimitIntegrationTests</c> (Fase 4.10). Los 5
/// endpoints restantes quedan cubiertos por inspección de código + el mismo evaluador ya
/// verificado en Fase 4.1-4.10. Cada test method usa un tenantId/userId nuevo
...
/// </summary>
public sealed class RateLimitIntegrationTests : IClassFixture<BillingApiFactory>
{
    private readonly BillingApiFactory factory;

    public RateLimitIntegrationTests(BillingApiFactory factory) => this.factory = factory;

    [Fact]
    public async Task ListInvoices_trips_with_user_layer_and_limit_300()
    {
        var tenantId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        var client = CreateAuthenticatedClient(tenantId, userId);

        var tripped = await FireUntilTrippedAsync(() => client.GetAsync("/billing/invoices?take=10"), maxAttempts: 600);

        await AssertTripped(
            tripped,
            expectedPolicy: "billing.f.invoice_read",
            expectedLayer: "user",
            expectedLimit: 300
        );
    }
    ...
    [Fact]
    public async Task UpsertIssuerProfile_trips_with_user_layer_and_limit_60()
    {
        var tenantId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        var client = CreateAuthenticatedClient(tenantId, userId);
        ...
        var tripped = await FireUntilTrippedAsync(
            () => client.PutAsJsonAsync("/billing/issuer-profile", new { Name = "Test Co" }),
            maxAttempts: 120
        );

        await AssertTripped(
            tripped,
            expectedPolicy: "billing.g.issuer_profile_manage",
            expectedLayer: "user",
            expectedLimit: 60
        );
    }
