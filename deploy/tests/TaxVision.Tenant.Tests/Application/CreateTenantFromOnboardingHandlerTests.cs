...
/// defensa en profundidad vía IAuthOnboardingStatusClient antes de crear el tenant real.</summary>
public sealed class CreateTenantFromOnboardingHandlerTests
{
    ...
    public async Task Creates_the_tenant_and_publishes_both_events_when_onboarding_is_ready()
    {
        ...
        var onboardingStatus = new FakeAuthOnboardingStatusClient(
            Result.Success(new OnboardingStatusSnapshot("Provisioning", DateTime.UtcNow))
        );
    ...
    public async Task Is_idempotent_when_a_tenant_already_exists_for_the_onboarding()
    {
        ...
        var onboardingStatus = new FakeAuthOnboardingStatusClient(
            Result.Failure<OnboardingStatusSnapshot>(new Error("should-not-be-called", "boom"))
        );
        var unitOfWork = new FakeUnitOfWork();
        var bus = new FakeMessageBus();
    ...
    [Fact]
    public async Task Fails_without_creating_a_tenant_when_the_onboarding_is_not_ready()
    {
        var repo = new FakeTenantRepository();
        var onboardingStatus = new FakeAuthOnboardingStatusClient(
            Result.Success(new OnboardingStatusSnapshot("PaymentProcessing", null))
        );
        var unitOfWork = new FakeUnitOfWork();
        var bus = new FakeMessageBus();

        var result = await CreateTenantFromOnboardingHandler.Handle(
            new CreateTenantFromOnboardingCommand(Guid.NewGuid(), "Acme Tax Office", "acme-tax", "owner@acme.com"),
            repo,
            onboardingStatus,
            unitOfWork,
            bus,
    ...
    private sealed class FakeAuthOnboardingStatusClient(Result<OnboardingStatusSnapshot> result)
        : IAuthOnboardingStatusClient
    {
        public Task<Result<OnboardingStatusSnapshot>> GetStatusAsync(
            Guid onboardingId,
