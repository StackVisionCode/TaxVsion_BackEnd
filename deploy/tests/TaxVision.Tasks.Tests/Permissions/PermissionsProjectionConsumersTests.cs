...
using TaxVision.Tasks.Application.Permissions.Consumers;
using TaxVision.Tasks.Domain.Permissions;

namespace TaxVision.Tasks.Tests.Permissions;

/// <summary>
/// Upsert idempotente por versión y unión de permisos de un usuario multi-rol. Fakes de mano, sin
/// Moq: se llama <c>Handle(...)</c> directo.
/// </summary>
public sealed class PermissionsProjectionConsumersTests
{
    ...
    public async Task UserRolesChanged_creates_projection_when_none_exists()
    {
        ...
        var evt = new UserRolesChangedIntegrationEvent
        {
            ...
            PermissionCodes = [TasksPermissions.Read],
            RoleIds = [roleId],
        ...
        Assert.Equal([TasksPermissions.Read], stored.PermissionCodes());
    ...
    public async Task UserRolesChanged_applies_newer_version_to_existing_projection()
    {
        ...
        var evt = new UserRolesChangedIntegrationEvent
        {
            ...
            PermissionCodes = [TasksPermissions.Read, TasksPermissions.Write],
            RoleIds = [],
            ActorType = "TenantEmployee",
        };
        ...
        Assert.Equal([TasksPermissions.Read, TasksPermissions.Write], stored.PermissionCodes());
    }

    /// <summary>Un redelivery fuera de orden no puede quitarle permisos al usuario en silencio.</summary>
    [Fact]
    public async Task UserRolesChanged_ignores_out_of_order_event_with_older_permissions_version()
    {
        ...
        var existing = UserPermissionsProjection.Create(
            ...
            userId,
            5,
            [TasksPermissions.Read, TasksPermissions.Write],
            []
        );
        var repo = new RecordingUserRepository(existing);
        var uow = new NoOpUnitOfWork();
    ...
    /// <summary>
    /// Un supervisor tiene el rol de empleado y además el de supervisión. Sobrescribir con los
    ...
    public async Task RolePermissionsChanged_recomputes_union_for_multi_role_affected_user()
    {
        ...
        var evt = new RolePermissionsChangedIntegrationEvent
        {
            ...
            RoleName = "Supervisor",
            PermissionsVersion = 1,
            PermissionCodes = [TasksPermissions.ManageAll],
        };
    ...
    public async Task RolePermissionsChanged_ignores_out_of_order_event_for_the_role_cache()
    {
        ...
        var existing = RolePermissionsProjection.Create(
            ...
            "Supervisor",
            5,
            [TasksPermissions.Read, TasksPermissions.ManageAll]
        );
        ...
        Assert.Equal(5, stored!.PermissionsVersion);
        Assert.Equal([TasksPermissions.Read, TasksPermissions.ManageAll], stored.PermissionCodes());
    }

    /// <summary>
    /// Un evento del tenant B no puede tocar la fila del tenant A aunque coincida el <c>UserId</c>:
    /// la proyección se escribe desde el scope de Wolverine, donde el filtro global no aplica.
    /// </summary>
    [Fact]
    public async Task UserRolesChanged_of_another_tenant_never_touches_this_tenants_row()
    {
        var mine = Guid.NewGuid();
        var theirs = Guid.NewGuid();
        var userId = Guid.NewGuid();
        var existing = UserPermissionsProjection.Create(mine, userId, 3, [TasksPermissions.ManageAll], []);
        var repo = new RecordingUserRepository(existing);
        var uow = new NoOpUnitOfWork();
        ...
        var untouched = await repo.GetAsync(mine, userId);
        Assert.Equal(3, untouched!.PermissionsVersion);
