...
using TaxVision.Reminder.Application.RateLimiting.Consumers;
using TaxVision.Reminder.Domain.RateLimiting;

namespace TaxVision.Reminder.Tests.RateLimiting;

/// <summary>
/// El consumer delega en el handler compartido de BuildingBlocks; lo que se prueba acá es que la
/// factory local (<c>TenantPlanCodeProjection.Create</c>) y el guard de revisión encajan bien con
/// él, y que la caché se invalida — sin eso el plan viejo sobrevive hasta que expire el TTL y el
/// tenant sigue con la cuota anterior aunque ya cambió de plan.
/// </summary>
