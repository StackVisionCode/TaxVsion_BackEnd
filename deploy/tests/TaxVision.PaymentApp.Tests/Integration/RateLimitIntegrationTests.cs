...
/// <summary>
/// Fase 4.13 del plan de rate limiting (Plan_Implementacion_Fases.md §4) — prueba end-to-end real
/// contra PaymentApp.Api: SQL Server/Redis/RabbitMQ locales reales, sin mocks de infraestructura.
/// Un test por categoría real de este servicio (F, G) — mismo criterio de cobertura que
/// <c>TaxVision.Auth.Tests.Integration.RateLimitIntegrationTests</c> (Fase 4.12). Los 9 endpoints
/// [RateLimit] restantes y los 2 [RateLimitExempt] quedan cubiertos por inspección de código + el
/// mismo evaluador ya verificado en Fase 4.1-4.12. PaymentAppAdminController se eligió porque
/// PlatformAdmin bypassea [HasPermission] (Program.cs: "los admins pasan siempre"), así que un
/// tenant/user sintético nuevo no choca contra RBAC. Cada test method usa un tenantId/userId nuevo
...
/// </summary>
public sealed class RateLimitIntegrationTests : IClassFixture<PaymentAppApiFactory>
{
    private readonly PaymentAppApiFactory factory;

    public RateLimitIntegrationTests(PaymentAppApiFactory factory) => this.factory = factory;
    ...
    public async Task SearchAllTenants_trips_with_user_layer_and_limit_300()
    {
        ...
        var tripped = await FireUntilTrippedAsync(
            () => client.GetAsync("/payments-app/admin/payments?page=1&pageSize=10"),
            maxAttempts: 600
        );

        await AssertTripped(
            tripped,
            expectedPolicy: "payment_app.f.admin_read",
            expectedLayer: "user",
            expectedLimit: 300
        );
    }
    ...
    [Fact]
    public async Task RepublishOnboardingResult_trips_with_user_layer_and_limit_60()
    {
        var tenantId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        var client = CreateAuthenticatedClient(tenantId, userId);
        ...
        var tripped = await FireUntilTrippedAsync(
            () =>
                client.PostAsync(
                    $"/payments-app/admin/payments/{paymentId}/republish-onboarding-result",
                    content: null
                ),
            maxAttempts: 120
        );
        ...
        await AssertTripped(
            tripped,
            expectedPolicy: "payment_app.g.admin_manage",
            expectedLayer: "user",
            expectedLimit: 60
        );
    }
