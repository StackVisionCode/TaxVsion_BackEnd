...
namespace TaxVision.Connectors.Tests.Integration;

/// <summary>
/// Fase 4.8 del plan de rate limiting (Plan_Implementacion_Fases.md §4) — prueba end-to-end real
/// contra Connectors.Api: SQL Server/Redis/RabbitMQ locales reales, sin mocks de infraestructura.
/// Un test por categoría real del inventario HTTP de este servicio (F, G) — mismo criterio de
/// cobertura que <c>TaxVision.Signature.Tests.Integration.RateLimitIntegrationTests</c> (Fase
/// 4.7). <c>connectors.i.accounts_manual_connect</c> queda fuera de este archivo por necesitar un
/// servidor IMAP+SMTP real que validar — cubierto por inspección de código + el mismo evaluador
/// ya verificado en Fase 4.1-4.7. Cada test method usa un tenantId/userId nuevo
...
/// </summary>
public sealed class RateLimitIntegrationTests : IClassFixture<ConnectorsApiFactory>
{
    private readonly ConnectorsApiFactory factory;

    public RateLimitIntegrationTests(ConnectorsApiFactory factory) => this.factory = factory;

    [Fact]
    public async Task AccountsList_trips_with_user_layer_and_limit_300()
    {
        var tenantId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        var client = CreateAuthenticatedClient(tenantId, userId);

        var tripped = await FireUntilTrippedAsync(() => client.GetAsync("/connectors/accounts"), maxAttempts: 600);

        await AssertTripped(
            tripped,
            expectedPolicy: "connectors.f.accounts_read",
            expectedLayer: "user",
            expectedLimit: 300
        );
    }
    ...
    [Fact]
    public async Task AdminConsentUrl_trips_with_user_layer_and_limit_60()
    {
        var tenantId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        var client = CreateAuthenticatedClient(tenantId, userId);
        ...
        var tripped = await FireUntilTrippedAsync(
            () => client.GetAsync("/connectors/accounts/admin-consent-url"),
            maxAttempts: 120
        );

        await AssertTripped(
            tripped,
            expectedPolicy: "connectors.g.accounts_manage",
            expectedLayer: "user",
            expectedLimit: 60
        );
    }
