...
using TaxVision.Notes.Application.Permissions.Consumers;
using TaxVision.Notes.Domain.Permissions;

namespace TaxVision.Notes.Tests.Permissions;

/// <summary>
/// Tests unitarios de <c>PermissionsProjectionConsumers.cs</c> (RBAC Fase 3 de este plan, copiado
/// del shape de CloudStorage/Growth) — upsert idempotente por versión + unión de permisos para un
/// usuario multi-rol. Fakes locales de mano, sin Moq — llama <c>Handle(...)</c> directo con
/// repos/UoW/correlation-context fake.
/// </summary>
public sealed class PermissionsProjectionConsumersTests
{
    ...
    public async Task UserRolesChanged_creates_projection_when_none_exists()
    {
        ...
        Assert.Equal(["notes.read"], stored.PermissionCodes());
    ...
    public async Task RolePermissionsChanged_recomputes_union_for_multi_role_affected_user()
    {
        ...
        var otherRole = RolePermissionsProjection.Create(tenantId, otherRoleId, "Viewer", 1, ["notes.read"]);
        var roleRepo = new RecordingRoleRepository(otherRole);

        var user = UserPermissionsProjection.Create(tenantId, userId, 1, ["notes.read"], [changedRoleId, otherRoleId]);
        ...
        Assert.Equal(["notes.manage"], storedRole!.PermissionCodes());
        ...
        Assert.Contains("notes.manage", storedUser!.PermissionCodes());
        Assert.Contains("notes.read", storedUser.PermissionCodes());
    ...
    public async Task RolePermissionsChanged_with_no_affected_users_still_saves_the_role_projection()
    {
        ...
        var evt = new RolePermissionsChangedIntegrationEvent
        {
            ...
            RoleName = "Empty",
            PermissionsVersion = 1,
            PermissionCodes = ["notes.view_all"],
        };
