...
> **Revisión 2026-09-16 — Wallet DIFERIDO (va, pero no ahora).** Este documento describe la saga **completa CON Wallet** (`reserve → dispatch → consume/refund`), que es el **estado FUTURO**. En **la fase actual (Wallet-free)** el protocolo se reduce a **`resolver audiencia → dispatch fan-out → aplicar result → agregar → cerrar run`**, sin reserve/consume/refund ni las invariantes I1/I2/I4 de saldo. El contrato dispatch/result y las invariantes de idempotencia (I3/I5/I6) siguen **idénticos**. Ver la versión vigente en `campaigns/Commands_And_Events.md §4` y `campaigns/State_Machines.md §2`. **Corrección 2026-09-16 (D7):** cuando entre el dinero, **NO** se reinsertan `Reserving`/`Reconciling` dentro de Campaign (Campaign nunca gana estados/columnas de dinero). En su lugar, un **interceptor de autorización (PEP) externo** frena/autoriza los triggers según saldo y el **Wallet** mide por fuera suscribiéndose a los eventos de Campaign. Por tanto la saga `reserve→consume/refund` que describe este doc pertenece al **Wallet/PEP externos**, no a la máquina de estados de Campaign. Ver `05_Master_ADR.md` D7 y `campaigns/Domain_Design.md §8.1`.
