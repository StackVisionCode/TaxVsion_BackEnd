...
public sealed class StripeConnectGateway : IStripeConnectGateway
{
    ...
    public async Task<Result<string>> CreateAccountAsync(ConnectAccountType type, string tenantEmail, CancellationToken ct)
    {
        ...
        catch (StripeException ex)
        {
            ...
            return Result.Failure<string>(new Error("StripeConnect.Account.CreateFailed", ex.StripeError?.Message ?? ex.Message));
        }
    }

    public async Task<Result<string>> CreateOnboardingLinkAsync(string stripeConnectAccountId, string refreshUrl, string returnUrl, CancellationToken ct)
    {
        ...
        {
            _logger.LogWarning(ex, "Stripe Connect CreateOnboardingLink failed for account {Account}", stripeConnectAccountId);
            return Result.Failure<string>(new Error("StripeConnect.OnboardingLink.CreateFailed", ex.StripeError?.Message ?? ex.Message));
        }
    }

    public async Task<Result<ConnectAccountStatusSnapshot>> GetAccountStatusAsync(string stripeConnectAccountId, CancellationToken ct)
    {
        var service = new AccountService(_client);
        try
        {
            ...
            return Result.Success(new ConnectAccountStatusSnapshot(
        ...
        {
            _logger.LogWarning(ex, "Stripe Connect GetAccountStatus failed for account {Account}", stripeConnectAccountId);
            return Result.Failure<ConnectAccountStatusSnapshot>(new Error("StripeConnect.Account.StatusFailed", ex.StripeError?.Message ?? ex.Message));
        }
    }
        ...
        string rawPayload, string signatureHeader, string webhookSecret, CancellationToken ct)
    {
        ...
        catch (StripeException ex)
        {
            ...
            return Task.FromResult(Result.Failure<ConnectWebhookEvent>(new Error("StripeConnect.WebhookSignature.Invalid", ex.Message)));
        }
        ...
        var result = stripeEvent.Type switch
        {
            ...
            _ => Result.Failure<ConnectWebhookEvent>(new Error("StripeConnect.Webhook.UnsupportedEventType", $"Event type '{stripeEvent.Type}' is not handled.")),
        };
    ...
    private static Result<ConnectWebhookEvent> ParseAccountEvent(Event stripeEvent)
    {
        if (stripeEvent.Data.Object is not Account account)
            return Result.Failure<ConnectWebhookEvent>(new Error("StripeConnect.Webhook.UnexpectedPayload", "Expected an Account object."));

        return Result.Success(new ConnectWebhookEvent(
            stripeEvent.Id, stripeEvent.Type, account.Id,
            ChargesEnabled: account.ChargesEnabled,
            PayoutsEnabled: account.PayoutsEnabled,
            RequirementsCurrentlyDue: account.Requirements?.CurrentlyDue ?? [],
            PayoutReference: null, PayoutAmountCents: null, PayoutCurrency: null, PayoutFailureReason: null));
    }
    ...
    private static Result<ConnectWebhookEvent> ParsePayoutEvent(Event stripeEvent, bool failed)
    {
        if (stripeEvent.Data.Object is not Payout payout)
            return Result.Failure<ConnectWebhookEvent>(new Error("StripeConnect.Webhook.UnexpectedPayload", "Expected a Payout object."));

        if (string.IsNullOrWhiteSpace(stripeEvent.Account))
            return Result.Failure<ConnectWebhookEvent>(new Error("StripeConnect.Webhook.MissingAccount", "Payout event did not carry a connected account id."));

        return Result.Success(new ConnectWebhookEvent(
            stripeEvent.Id, stripeEvent.Type, stripeEvent.Account,
            ChargesEnabled: null, PayoutsEnabled: null, RequirementsCurrentlyDue: null,
    ...
    private static string MapAccountType(ConnectAccountType type) => type switch
    {
