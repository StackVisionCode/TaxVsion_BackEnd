...
public sealed class PayoutScheduleRepository(PaymentClientDbContext db) : IPayoutScheduleRepository
{
    public Task<PayoutSchedule?> GetByTenantAsync(Guid tenantId, CancellationToken ct = default) =>
        db.PayoutSchedules.Include(schedule => schedule.Items).FirstOrDefaultAsync(schedule => schedule.TenantId == tenantId, ct);

    public Task<PayoutSchedule?> GetByTenantConnectAccountIdAsync(Guid tenantConnectAccountId, CancellationToken ct = default) =>
        db.PayoutSchedules.Include(schedule => schedule.Items).FirstOrDefaultAsync(schedule => schedule.TenantConnectAccountId == tenantConnectAccountId, ct);
