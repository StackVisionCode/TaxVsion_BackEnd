...
public sealed class TenantPaymentConfiguration : IEntityTypeConfiguration<TenantPayment>
{
    public void Configure(EntityTypeBuilder<TenantPayment> builder)
    {
        ...
        builder.OwnsOne(payment => payment.Purpose, purpose =>
        {
        ...
        builder.OwnsOne(payment => payment.ExternalChargeReference, reference =>
        {
            reference.Property(r => r.Provider).HasColumnName("ExternalChargeProvider").HasConversion<string>().HasMaxLength(30);
            reference.Property(r => r.Value).HasColumnName("ExternalChargeReference").HasMaxLength(200);
        });

        builder.Property(payment => payment.ProviderChargeReferenceOnConnect).HasColumnName("ProviderChargeReferenceOnConnect").HasMaxLength(200);

        builder.OwnsOne(payment => payment.SplitPayment, split =>
        {
        ...
        builder.Property(payment => payment.StatementDescriptor)
        ...
        builder.HasIndex(payment => new { payment.TenantId, payment.Status })
            .HasDatabaseName("IX_TenantPayments_TenantId_Status");

        builder.HasIndex(payment => payment.NextRetryAtUtc)
