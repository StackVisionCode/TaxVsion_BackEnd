...
public sealed record Money
{
    ...
    public static Result<Money> Create(long amountCents, string currency)
    {
        ...
        if (string.IsNullOrWhiteSpace(currency) || currency.Trim().Length != 3)
            return Result.Failure<Money>(new Error("Money.InvalidCurrency", "Currency must be a 3-letter ISO-4217 code."));
    ...
    public Result<Money> Add(Money other)
    {
        if (other.Currency != Currency)
            return Result.Failure<Money>(new Error("Money.CurrencyMismatch", "Cannot add amounts in different currencies."));
    ...
    {
        if (other.Currency != Currency)
            return Result.Failure<Money>(new Error("Money.CurrencyMismatch", "Cannot subtract amounts in different currencies."));
