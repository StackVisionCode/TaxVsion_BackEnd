...
public static class GetTenantRecurringPaymentByIdHandler
{
    public static async Task<Result<TenantRecurringPaymentResponse>> Handle(
        GetTenantRecurringPaymentByIdQuery query, ITenantRecurringPaymentRepository plans, CancellationToken ct)
    {
        var plan = await plans.GetByIdAsync(query.TenantRecurringPaymentId, query.TenantId, ct);
        if (plan is null)
            return Result.Failure<TenantRecurringPaymentResponse>(new Error("TenantRecurringPayment.NotFound", "TenantRecurringPayment does not exist."));

        return Result.Success(Map(plan));
    }

    internal static TenantRecurringPaymentResponse Map(TenantRecurringPayment plan) => new(
        plan.Id,
        plan.TaxpayerId,
        ...
        plan.FailureCount,
        plan.Schedules
            .Select(s => new RecurringScheduleResponse(
                s.Id, s.ScheduledDate, s.Status.ToString(), s.Amount.AmountCents, s.Amount.Currency, s.TenantPaymentId, s.RetryCount, s.NextRetryAtUtc))
