...
public static class SearchTenantRecurringPaymentsHandler
{
    public static async Task<Result<IReadOnlyList<TenantRecurringPaymentResponse>>> Handle(
        SearchTenantRecurringPaymentsQuery query,
        ITenantRecurringPaymentRepository plans,
        CancellationToken ct
    )
    {
        var results = await plans.SearchByTenantAsync(
            query.TenantId,
            query.TaxpayerId,
            query.Status,
            query.Page,
        ...
        return Result.Success<IReadOnlyList<TenantRecurringPaymentResponse>>(
            results.Select(GetTenantRecurringPaymentByIdHandler.Map).ToList()
        );
    }
}
