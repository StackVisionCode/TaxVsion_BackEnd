...
public static class CreatePaymentLinkHandler
{
        ...
        CancellationToken ct)
    {
        ...
        await AuditEntryFactory.AppendAsync(
            audit, command.TenantId, nameof(PaymentLink), link.Id, PaymentAuditAction.PaymentLinkCreated,
            command.ActorUserId, correlation.CorrelationId,
            before: (object?)null,
            after: new { link.Amount.AmountCents, link.Amount.Currency, link.ExpiresAtUtc },
            reason: null, nowUtc, ct);

        await bus.PublishAsync(new PaymentLinkCreatedIntegrationEvent
        {
