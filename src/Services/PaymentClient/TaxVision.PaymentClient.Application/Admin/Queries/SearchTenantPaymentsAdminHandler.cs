...
public static class SearchTenantPaymentsAdminHandler
{
    public static async Task<Result<IReadOnlyList<TenantPaymentAdminResponse>>> Handle(
        SearchTenantPaymentsAdminQuery query, ITenantPaymentRepository payments, CancellationToken ct)
    {
        var results = await payments.SearchAdminAsync(query.TenantId, query.Status, query.From, query.To, query.Page, query.PageSize, ct);

        return Result.Success<IReadOnlyList<TenantPaymentAdminResponse>>(results.Select(Map).ToList());
    }

    private static TenantPaymentAdminResponse Map(TenantPayment payment) => new(
        payment.Id,
        payment.TenantId,
