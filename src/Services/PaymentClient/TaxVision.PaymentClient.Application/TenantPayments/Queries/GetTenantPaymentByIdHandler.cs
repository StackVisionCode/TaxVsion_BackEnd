...
public static class GetTenantPaymentByIdHandler
{
    public static async Task<Result<TenantPaymentResponse>> Handle(
        GetTenantPaymentByIdQuery query, ITenantPaymentRepository payments, CancellationToken ct)
    {
        var payment = await payments.GetByIdAsync(query.TenantPaymentId, query.TenantId, ct);
        if (payment is null)
            return Result.Failure<TenantPaymentResponse>(new Error("TenantPayment.NotFound", "TenantPayment does not exist."));

        return Result.Success(new TenantPaymentResponse(
            payment.Id,
