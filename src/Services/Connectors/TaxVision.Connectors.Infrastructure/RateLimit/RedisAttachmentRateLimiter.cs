...
/// <summary>Ventana fija de 1 minuto por (tenant, cuenta), igual que RedisMessageBodyRateLimiter — comparte presupuesto entre réplicas. Fail-fast.</summary>
    ...
    : IAttachmentRateLimiter
{
    public async Task<bool> TryAcquireAsync(Guid tenantId, Guid accountId, CancellationToken ct = default)
    {
        ...
        var key = RateCounterKey.From($"connectors:attachment-fetch:{tenantId:N}:{accountId:N}:window:{minuteBucket}");
