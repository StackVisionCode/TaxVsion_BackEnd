...
public sealed class TenantSubscriptionConfiguration : IEntityTypeConfiguration<TenantSubscription>
{
    public void Configure(EntityTypeBuilder<TenantSubscription> builder)
    {
        ...
        builder
            .HasIndex(subscription => subscription.NextRenewalAtUtc)
        ...
        builder
            .HasOne<SubscriptionPlanVersion>()
            .WithMany()
            .HasForeignKey(subscription => subscription.PlanVersionId)
            .OnDelete(DeleteBehavior.Restrict);

        builder
            .HasMany(subscription => subscription.Renewals)
            .WithOne()
        ...
        builder
            .HasMany(subscription => subscription.PlanChangeRequests)
            .WithOne()
        ...
        builder
            .HasMany(subscription => subscription.PendingDowngrades)
            .WithOne()
            .HasForeignKey(pending => pending.TenantSubscriptionId)
            .OnDelete(DeleteBehavior.Cascade);
        builder
            .Navigation(subscription => subscription.PendingDowngrades)
            .UsePropertyAccessMode(PropertyAccessMode.Field);
    }
}
