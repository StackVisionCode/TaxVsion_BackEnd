...
public static class AddOnRenewalPaymentSucceededConsumer
{
    ...
    )
    {
        ...
        using (correlation.Push(correlationId))
        {
            ...
            if (addOn is null)
            {
                logger.LogWarning("AddOnRenewalPaymentSucceeded for unknown add-on {TenantAddOnId}.", evt.TenantAddOnId);
                return;
            }
            ...
            {
                logger.LogWarning(
                    "AddOnRenewalPaymentSucceeded for {TenantAddOnId} has no matching renewal for key {Key}.", evt.TenantAddOnId, evt.IdempotencyKey);
                return;
            }

            var result = addOn.CompleteRenewal(renewal.Id, evt.ExternalPaymentReference, actorUserId: Guid.Empty, evt.PaidAtUtc);
            if (result.IsFailure)
            {
                logger.LogWarning("Could not complete renewal for add-on {TenantAddOnId}: {Code}.", addOn.Id, result.Error.Code);
                return;
            }
