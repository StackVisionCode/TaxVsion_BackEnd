...
public sealed class SubscriptionsController(IMessageBus bus) : ControllerBase
{
    ...
    public async Task<IActionResult> GetMySubscription(CancellationToken ct)
    {
        ...
        var result = await bus.InvokeAsync<Result<MySubscriptionResponse>>(
            new GetMySubscriptionQuery(tenantId), ct);
        ...
        CancellationToken ct)
    {
        ...
        var result = await bus.InvokeAsync<Result>(
            new ChangePlanCommand(tenantId, request.PlanCode, userId), ct);
    ...
    [Authorize(Roles = "TenantAdmin")]
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    public async Task<IActionResult> PurchaseSeats(
        ...
        CancellationToken ct)
    {
        ...
        var result = await bus.InvokeAsync<Result>(
            new PurchaseSeatsCommand(tenantId, request.AdditionalSeats, userId), ct);
    ...
    public async Task<IActionResult> Cancel(CancellationToken ct)
    {
        ...
        var result = await bus.InvokeAsync<Result>(
            new CancelSubscriptionCommand(tenantId, userId), ct);
    ...
    {
        var result = await bus.InvokeAsync<Result>(
            new SuspendSubscriptionCommand(tenantId, request.Reason), ct);
    ...
    {
        var result = await bus.InvokeAsync<Result>(
            new ReactivateSubscriptionCommand(tenantId), ct);
