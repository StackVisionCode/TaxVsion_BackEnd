...
{
    public static async Task<bool> EvaluateAsync(User user, IMfaRepository mfa, CancellationToken ct)
    {
        if (user.MfaEnabled)
            return true;
    ...
    /// </summary>
    public static async Task<MfaDisposition> DisposeAsync(User user, IMfaRepository mfa, CancellationToken ct)
    {
        if (!await EvaluateAsync(user, mfa, ct))
