...
public sealed class TasksDbContextFactory : IDesignTimeDbContextFactory<TasksDbContext>
{
    public TasksDbContext CreateDbContext(string[] args)
    {
        ...
        var options = new DbContextOptionsBuilder<TasksDbContext>().UseSqlServer(connectionString).Options;

        // dotnet-ef solo inspecciona el modelo, nunca ejecuta una query real.
        return new TasksDbContext(options, new DesignTimeOnlyTenantContext());
    }
