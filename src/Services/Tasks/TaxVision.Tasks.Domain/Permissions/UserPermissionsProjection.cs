...
namespace TaxVision.Tasks.Domain.Permissions;

/// <summary>
/// Permisos efectivos por usuario. La lee <c>ProjectionPermissionsSource</c> para autorizar sin
/// llamar a Auth. <c>RoleIds</c> permite recomponer la unión de un usuario multi-rol cuando cambia
/// sólo uno de sus roles.
/// </summary>
public sealed class UserPermissionsProjection : TenantEntity
{
    ...
    /// <summary>
    /// Idempotente por la versión monotónica de Auth. Sin comparar <c>PermissionsVersion</c> un
    /// evento fuera de orden deja al usuario fail-closed en silencio, sin DLQ.
    /// </summary>
    ...
    /// permisos filtra por <c>IsActive</c>). No toca permisos ni versión — el evento no los trae.
    /// Idempotente.
    /// </summary>
