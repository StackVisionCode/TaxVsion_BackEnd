...
public static class GetTenantProviderCustomerHandler
{
    public static async Task<Result<TenantProviderCustomerResponse>> Handle(
        GetTenantProviderCustomerQuery query,
    ...
    )
    {
        ...
        if (customer is null)
            return Result.Failure<TenantProviderCustomerResponse>(
        ...
        foreach (var method in customer.SavedMethods)
        {
            if (!method.IsDetached)
                methods.Add(
                    new SavedPaymentMethodResponse(
                        method.Id,
                        method.Brand,
                        method.Last4,
                        method.ExpMonth,
        ...
        return Result.Success(
            new TenantProviderCustomerResponse(customer.Id, customer.ProviderCode.ToString(), customer.Email, methods)
        );
    }
}
