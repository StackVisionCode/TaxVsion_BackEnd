...
public static class GetSaaSPaymentByIdHandler
{
    ...
    )
    {
        ...
        if (payment is null)
            return Result.Failure<SaaSPaymentResponse>(
                new Error("SaaSPayment.NotFound", "SaaSPayment does not exist.")
            );

        return Result.Success(
            new SaaSPaymentResponse(
                payment.Id,
