...
{
    Task<TenantProviderCustomer?> GetByTenantAndProviderAsync(
        Guid tenantId,
        PaymentProviderCode code,
        CancellationToken ct = default
    );
    Task<TenantProviderCustomer?> GetByIdAsync(
    ...
    Task<IReadOnlyList<TenantProviderCustomer>> GetWithMethodsExpiringBeforeAsync(
        DateTime cutoffUtc,
        int batchSize,
        CancellationToken ct = default
    );
