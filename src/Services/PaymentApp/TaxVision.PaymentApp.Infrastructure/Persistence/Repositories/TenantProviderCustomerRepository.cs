...
{
    public Task<TenantProviderCustomer?> GetByTenantAndProviderAsync(Guid tenantId, PaymentProviderCode code, CancellationToken ct = default) =>
    ...
    public Task<TenantProviderCustomer?> GetByIdAsync(Guid tenantProviderCustomerId, Guid tenantId, CancellationToken ct = default) =>
        WithMethods(db.TenantProviderCustomers)
            .FirstOrDefaultAsync(customer => customer.Id == tenantProviderCustomerId && customer.TenantId == tenantId, ct);
    ...
    public async Task<IReadOnlyList<TenantProviderCustomer>> GetWithMethodsExpiringBeforeAsync(
        ...
        await WithMethods(db.TenantProviderCustomers)
            .Where(customer => customer.SavedMethods.Any(method =>
                !method.IsDetached
                && method.ExpiryNoticeSentAtUtc == null
                && (method.ExpYear < cutoffUtc.Year || (method.ExpYear == cutoffUtc.Year && method.ExpMonth <= cutoffUtc.Month))))
