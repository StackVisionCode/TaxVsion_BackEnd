...
public sealed class ProviderWebhookSecrets(IOptions<StripeOptions> stripeOptions, IOptions<PayPalOptions> payPalOptions)
    : IProviderWebhookSecrets
{
    ...
    public string? GetWebhookId(PaymentProviderCode code) =>
        code switch
        {
            PaymentProviderCode.PayPal => payPalOptions.Value.WebhookId,
            _ => null,
        };
}
