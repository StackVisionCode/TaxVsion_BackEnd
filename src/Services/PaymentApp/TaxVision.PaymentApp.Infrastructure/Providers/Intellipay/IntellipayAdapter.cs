...
public sealed class IntellipayAdapter(IntellipayGateway gateway, ILogger<IntellipayAdapter> logger) : IPaymentProvider
{
        ...
        Guid tenantId, string email, string? name, CancellationToken ct)
    {
        var response = await gateway.CreateCustomerAsync(
            new IntellipayCreateCustomerRequest(Account: tenantId.ToString("N"), Email: email, FirstName: name ?? "TaxVision Customer"),
            ct);

        if (!response.IsApproved || string.IsNullOrWhiteSpace(response.CustId))
        {
            logger.LogWarning("Intellipay create_customer failed for tenant {TenantId}: {Message}", tenantId, response.Message);
            return Result.Failure<ProviderCustomerToken>(new Error("Intellipay.CreateCustomer.Failed", response.Message ?? "Unknown Intellipay error."));
        }
    ...
    public async Task<Result<RefundResult>> RefundAsync(string providerChargeReference, Money amount, string reason, CancellationToken ct)
    {
        var response = await gateway.RefundAsync(
            new IntellipayRefundRequest(TransactionId: providerChargeReference, AmountCents: amount.AmountCents, Reason: reason), ct);

        if (!response.IsApproved)
        {
            logger.LogWarning("Intellipay refund failed for {Reference}: {Message}", providerChargeReference, response.Message);
            return Result.Failure<RefundResult>(new Error("Intellipay.Refund.Failed", response.Message ?? "Unknown Intellipay error."));
        }

        return Result.Success(new RefundResult(response.TransactionId ?? providerChargeReference, PaymentStatus.Refunded, amount));
    }
    ...
    public Task<Result<WebhookVerificationResult>> VerifyWebhookSignatureAsync(
        ...
        Task.FromResult(Result.Failure<WebhookVerificationResult>(
            new Error("Intellipay.WebhookSignature.NotSupported", "Intellipay does not sign IPN callbacks. Use IntellipayIpnValidator instead.")));
    ...
    public Task<Result<WebhookEventPayload>> ParseWebhookEventAsync(string rawPayload, string eventType, CancellationToken ct) =>
        Task.FromResult(Result.Failure<WebhookEventPayload>(
            new Error("Intellipay.Webhook.NotSupported", "Intellipay IPN parsing is handled by IntellipayIpnValidator, not this generic path.")));
    ...
    public Task<Result<SavedPaymentMethodInfo>> AttachPaymentMethodAsync(
        ...
        Task.FromResult(Result.Failure<SavedPaymentMethodInfo>(
            new Error("Intellipay.PaymentMethod.NotSupported", "Intellipay does not support attaching additional payment methods yet.")));

    public Task<Result> DetachPaymentMethodAsync(string paymentMethodReference, CancellationToken ct) =>
        Task.FromResult(Result.Failure(new Error("Intellipay.PaymentMethod.NotSupported", "Intellipay does not support detaching payment methods yet.")));
}
