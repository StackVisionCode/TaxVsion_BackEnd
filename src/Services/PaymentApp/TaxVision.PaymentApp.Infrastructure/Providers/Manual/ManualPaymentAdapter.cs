...
public sealed class ManualPaymentAdapter(ILogger<ManualPaymentAdapter> logger) : IPaymentProvider
{
    ...
    )
    {
        ...
        return Task.FromResult(
            Result.Success(
                new ChargeAuthorizationResult(ProviderChargeReference: reference, Status: PaymentStatus.Succeeded)
            )
        );
    }
    ...
    public Task<Result<WebhookVerificationResult>> VerifyWebhookSignatureAsync(
        ...
        Task.FromResult(
            Result.Failure<WebhookVerificationResult>(
                new Error(
                    "Manual.WebhookSignature.NotSupported",
                    "The Manual provider has no webhooks — payments are confirmed by an admin action."
                )
            )
        );
    ...
    public Task<Result<SetupIntentInfo>> CreateSetupIntentAsync(ProviderCustomerToken customer, CancellationToken ct) =>
        Task.FromResult(
            Result.Failure<SetupIntentInfo>(
                new Error(
                    "Manual.SetupIntent.NotSupported",
                    "The Manual provider has no client-side card tokenization."
                )
            )
        );
    ...
    public Task<Result<HostedCheckoutSessionResult>> CreateHostedCheckoutSessionAsync(
        ...
        Task.FromResult(
            Result.Failure<HostedCheckoutSessionResult>(
                new Error("Manual.CheckoutSession.NotSupported", "The Manual provider has no hosted checkout flow.")
            )
        );
}
