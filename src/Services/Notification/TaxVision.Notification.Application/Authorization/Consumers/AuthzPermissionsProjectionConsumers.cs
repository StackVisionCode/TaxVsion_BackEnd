...
namespace TaxVision.Notification.Application.Authorization.Consumers;
...
// HTTP en el hot path de autorización. Mismo patrón que CloudStorage/Customer/Scribe/Tenant:
...
//
// Deliberadamente en su propio namespace (Authorization, no Permissions) y con nombres
// "Authz*" — Notification ya tenía UserRolesChangedConsumer/RolePermissionsChangedConsumer en
// TaxVision.Notification.Application.Consumers (Fase 4 del plan de notificaciones dinámicas,
// alimentan IRecipientResolver/ByPermission, un propósito de negocio distinto: resolver
// audiencias de notificación, no autorizar la request). Wolverine invoca TODOS los handlers
// registrados para un mismo tipo de evento — ambos pares de consumers procesan
// UserRolesChangedIntegrationEvent/RolePermissionsChangedIntegrationEvent de forma
// independiente, cada uno escribiendo en su propia tabla, sin interferir entre sí.
// ---------------------------------------------------------------------------
