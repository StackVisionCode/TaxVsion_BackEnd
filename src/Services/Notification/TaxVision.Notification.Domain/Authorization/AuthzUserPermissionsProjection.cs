...
/// <summary>
/// Proyección local de permisos efectivos por usuario para AUTORIZACIÓN (RBAC Fase 7 —
/// enforcement de <c>perm_v</c>). La consulta <c>ProjectionPermissionsSource</c>
/// (BuildingBlocks.Web) vía el adaptador de Infrastructure que implementa
/// <c>BuildingBlocks.Permissions.IUserPermissionsProjectionReader</c>.
///
/// <para>
...
/// Mezclarlas acoplaría dos consumidores con necesidades de consistencia distintas (uno
/// tolera datos stale por unos segundos sin problema — un fan-out tarde no rompe nada crítico;
/// el otro es la puerta de un endpoint protegido) al mismo ciclo de vida. Mismo shape que la
/// proyección homónima de CloudStorage/Customer/Scribe/Tenant (RBAC Fase 7) — <c>RoleIds</c>
