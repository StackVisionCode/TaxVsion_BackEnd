...
import { connectPrisma, disconnectPrisma } from './infrastructure/persistence/prisma-client.js';
...
async function main(): Promise<void> {
  ...
  const missedCalls = startMissedCallScheduler(
    ...
    { calls: container.calls, publisher: container.publisher },
  );
  const outbox = startOutboxDrainer({ intervalMs: 2000 });
  ...
  bindAuthConsumers(consumers.register.bind(consumers), { userPermissions: container.userPermissions });
  ...
  for (const signal of signals) {
    process.once(signal, () => {
      void shutdown(signal, app, io, missedCalls, outbox, sessionWatcher);
    });
  }
}
