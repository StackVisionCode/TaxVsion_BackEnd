...
namespace TaxVision.Reminder.Domain.Permissions;
...
/// Mismo shape que la proyección homónima de Notes/CloudStorage/Signature — <c>RoleIds</c> permite
/// recomputar la unión de permisos de un usuario multi-rol cuando llega
...
public sealed class UserPermissionsProjection : TenantEntity
{
    ...
    /// <summary>
    /// Idempotente por versión monotónica emitida por Auth — eventos fuera de orden se ignoran.
    /// Comparar <c>PermissionsVersion</c> NO es opcional: el upsert de Communication no lo hacía y
    /// dejaba usuarios fail-closed en silencio, sin DLQ.
    /// </summary>
    ...
    /// <summary>
    /// RolePermissionsChangedIntegrationEvent para UNO de los roles de este usuario: recompone la
    /// unión de permisos de todos sus roles cacheados, sin tocar PermissionsVersion (el cambio no
    /// vino de una reasignación de roles del propio usuario) ni RoleIds.
    /// </summary>
