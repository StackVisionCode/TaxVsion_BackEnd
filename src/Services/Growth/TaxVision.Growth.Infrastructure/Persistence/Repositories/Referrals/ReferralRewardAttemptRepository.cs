...
public sealed class ReferralRewardAttemptRepository(
        ...
        || !TenantRepositoryGuard.Matches(tenantContext, ownerTenantId)
            ...
            : dbContext.ReferralRewardAttempts.FirstOrDefaultAsync(
                attempt =>
                    ...
                    && attempt.TenantId == ownerTenantId,
                ct
            );

    public async Task AddAsync(
        ReferralRewardAttempt attempt,
        CancellationToken ct = default
    )
    {
        if (!TenantRepositoryGuard.Matches(tenantContext, attempt.TenantId))
        {
            var elevatedRewardIsTracked = tenantContext.HasTenant
                && tenantContext.TenantId != Guid.Empty
                && dbContext
                    .ChangeTracker.Entries<ReferralRewardCase>()
                    .Any(entry =>
                        entry.Entity.Id == attempt.RewardCaseId
                        && entry.Entity.TenantId == attempt.TenantId
                    );
