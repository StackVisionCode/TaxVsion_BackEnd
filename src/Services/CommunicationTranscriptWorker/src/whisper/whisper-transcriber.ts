 ...
 */
export async function transcribeWav(wavPath: string, outPrefix: string): Promise<TranscriptionResult> {
...
export function extractDetectedLanguage(stdout: string): string | null {
...
const LEADING_SEGMENT_TIMESTAMP_PATTERN =
  /^\s*\[\d{2}:\d{2}:\d{2}\.\d{3}\s*-->\s*\d{2}:\d{2}:\d{2}\.\d{3}\]\s*/;

/** Quita el prefijo `[hh:mm:ss.mmm --> hh:mm:ss.mmm]` de cada linea, si esta presente. */
export function stripSegmentTimestamps(rawText: string): string {
  return rawText
    .split(/\r?\n/)
...
export function parseDurationSeconds(stdout: string): number {
  ...
  const totalSeconds =
    Number(endH) * 3600 + Number(endM) * 60 + Number(endS) + Number(endMs) / 1000;
