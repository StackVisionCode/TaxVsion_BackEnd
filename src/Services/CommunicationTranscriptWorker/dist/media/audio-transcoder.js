 ...
 */
export async function probeAudioStreams(inputPath) {
    const { stdout } = await execFileAsync(config.ffmpeg.ffprobeBinPath, ['-v', 'error', '-select_streams', 'a', '-show_entries', 'stream=index', '-of', 'csv=p=0', inputPath], { timeout: 30_000 });
    const count = stdout
        .split('\n')
        .map((line) => line.trim())
