...
public interface IUserPermissionsProjectionRepository
{
    ...
    Task<IReadOnlyList<UserPermissionsProjection>> FindActiveByTenantAndRoleIdAsync(Guid tenantId, Guid roleId, CancellationToken ct = default);
}
...
public interface IRolePermissionsProjectionRepository
{
    ...
    Task<IReadOnlyList<RolePermissionsProjection>> FindByRoleIdsAsync(Guid tenantId, IReadOnlyCollection<Guid> roleIds, CancellationToken ct = default);
}
