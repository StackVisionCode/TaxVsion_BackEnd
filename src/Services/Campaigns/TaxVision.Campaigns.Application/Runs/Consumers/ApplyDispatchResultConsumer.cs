...
public static class ApplyDispatchResultConsumer
{
    ...
    )
    {
        ...
        {
            logger.LogWarning("Unknown dispatch outcome '{Outcome}' for run {RunId}; ignoring.", evt.Outcome, evt.RunId);
            return;
        }
        ...
        if (run is null)
        {
            logger.LogWarning("Dispatch result for unknown run {RunId} (tenant {TenantId}); ignoring.", evt.RunId, evt.TenantId);
            return;
        }
        ...
        {
            logger.LogWarning("Dispatch result for unknown unit {DispatchId} in run {RunId}; ignoring.", evt.DispatchId, evt.RunId);
            return;
        }
