...
namespace TaxVision.Documents.Application.Permissions.Consumers;
...
// perder permisos de un usuario multi-rol. Los eventos llegan por la cola documents-events (bindeada al
// exchange taxvision-events donde Auth publica). Mismo patrón que Signature/CloudStorage/etc.
...
public static class AuthzRolePermissionsChangedPermissionsProjectionConsumer
{
    ...
    /// <summary>
    /// Un usuario con VARIOS roles no puede sobrescribirse solo con los códigos del rol que cambió, o
    /// perdería los permisos heredados de sus otros roles — recompone la unión completa contra el cache
    /// de AuthzRolePermissionsProjection. <paramref name="changedRole"/> se inyecta directamente (en vez
    /// de reconsultarlo) porque su UPDATE todavía no se guardó en esta transacción.
    /// </summary>
